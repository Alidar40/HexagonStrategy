    ...
    public void Load(string saveName)
    {
        ...
        if (buf == "Construction")
            ...
            if (buf == "Unit")
            {
                ...
                {
                    buf = SaveStringArray[++StringCount];
                    Unit.CreateUnit(MapToSave.UnitPrefabArray[1], Unit.UnitType.Archer, Convert.ToInt32(SaveStringArray[++StringCount], 10), Convert.ToInt32(SaveStringArray[++StringCount], 10), MapToSave.UnitList, buf, Convert.ToInt32(SaveStringArray[++StringCount], 10));
                }
                if (buf == "Mage")
                {
                    buf = SaveStringArray[++StringCount];
